...
class guitar(smach.State):
    def __init__(self,robot):
        smach.State.__init__(self, outcomes=['succeeded'])
    def execute(self, userdata):
                robot.behavior_manager.run_behavior("dance_saxo/guitar")
                return 'succeeded'
...
def getInstance(robot):
    ...
    with sm:
        ...
        smach.StateMachine.add('Photo',ShowWebpage(robot,page="https://i.ytimg.com/vi/VVHSM5CYirI/maxresdefault.jpg",timeout=10),
            transitions={
        ...
        smach.StateMachine.add('guitar', guitar(robot),
            transitions={
                'succeeded':'succeeded'
            }        )
