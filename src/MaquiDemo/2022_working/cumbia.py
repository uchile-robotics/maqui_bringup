...
class Cumbia(smach.State):
    def __init__(self,robot):
        smach.State.__init__(self, outcomes=['succeeded'])
    def execute(self, userdata):
                robot.behavior_manager.run_behavior("baile-29c766/Cumbia")
                return 'succeeded'
...
def getInstance(robot):
    ...
    with sm:
        ...
        smach.StateMachine.add('Photo',ShowWebpage(robot,page="https://i.ytimg.com/vi/jsRMuxLFAb0/maxresdefault.jpg",timeout=10),
            transitions={
        ...
        smach.StateMachine.add('cumbia', Cumbia(robot),
            transitions={
                'succeeded':'succeeded'
            }        )
